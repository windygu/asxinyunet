...
namespace LotteryTicket
{
    ...
    public static class ValidateMethods
    {
        ...
        /// <param name="data">数据</param>
        /// <param name="rows">验证的行数</param>
        /// <param name="reapterCount">重复数,默认为2</param>
        /// <returns>在指定期内，只要有一期重复数大于等于reapterCount则失败，返回最终的准确率</returns>
        public static double RepeatNumbersValidate(double[][] data, int rows, int reapterCount = 2)
        {
            //单独写算法
            //然后再循环比较,先将所有结果一一对比，然后再根据期数去判断
            ...
            for (int i = 0; i < L; i++)//将连续的几期，逐一比较
            {
                ...
                for (int j = i + 1; j < i + rows; j++)
                {
                    if (dic[i.ToString() + "-" + j.ToString()] >= reapterCount)//说明不满足要求
                    {
                        temp = true; break;
                    }
                    if (temp) { break; }//如果不满足,也跳出循环
