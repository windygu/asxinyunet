...
private System.Windows.Forms.TextBox txtRemark ;
   ...
   private System.Windows.Forms.ErrorProvider errorProvider1 ;
		...
		private void InitializeComponent()
		{
            ...
            this.lblRawNameTP.Size = new System.Drawing.Size(63, 14);
            ...
            this.lblJQIsHaveKQTP.Size = new System.Drawing.Size(63, 14);
            ...
            this.lblCYIsHaveKQTP.Size = new System.Drawing.Size(63, 14);
            ...
            this.lblT8WIsHaveKQTP.Size = new System.Drawing.Size(70, 14);
            ...
            this.lblGetSampLocationTP.Size = new System.Drawing.Size(63, 14);
            ...
            this.Size = new System.Drawing.Size(317, 500);
		...
		#region 构造函数 及初始化
		public AddKqTestData()	{InitializeComponent(); CustomerSettings(); }	
		...
		/// 获取或者设置当前实体
		/// </summary>
		public tb_KqTestData CutModel{get ;set ;}
		/// <summary>
		/// 当前实体窗体的显示模式,默认为连续显示
		/// </summary>
		...
		bool ValidateControls()
		{
			...
			if(combGetSampLocationTP.Text.Trim()=="")//采样地点
			{
		...
		void btnOK_Click(object sender, EventArgs e)
		{
			...
			else 
			{
				if(((CutShowMode!= FormShowMode.ReadOnlyForAll) || (CutShowMode != FormShowMode.ReadOnlyForOne)) && ValidateControls() )
				{
					...
					if(combRawNameTP.Text.Trim()!="") model.RawNameTP = combRawNameTP.Text.Trim() ;//原料名称
					if(combJQIsHaveKQTP.Text.Trim()!="") model.JQIsHaveKQTP = combJQIsHaveKQTP.Text.Trim() ;//精油含醛
					if(txtAV.Text.Trim()!="")	model.AV =Convert.ToDouble(txtAV.Text.Trim()) ;//酸值
					if(combCYIsHaveKQTP.Text.Trim()!="") model.CYIsHaveKQTP = combCYIsHaveKQTP.Text.Trim() ;//抽油含醛
					if(combT8WIsHaveKQTP.Text.Trim()!="") model.T8WIsHaveKQTP = combT8WIsHaveKQTP.Text.Trim() ;//塔8水含醛
					...
					if(combGetSampLocationTP.Text.Trim()!="") model.GetSampLocationTP = combGetSampLocationTP.Text.Trim() ;//采样地点
		...
		List <tb_KqTestData> modelList ;//当前实体列表
		void GetData()
		{				
			//判断不为空，才能绑定				
		...
		private void SetControlReSet()
		{
			...
			if (CutShowMode == FormShowMode.AddOne || CutShowMode== FormShowMode.ContinueAdd )
			{
				...
				combRawNameTP.Enabled = true ; //原料名称	
				combJQIsHaveKQTP.Enabled = true ; //精油含醛	
				txtAV.ReadOnly = false ;//酸值
				txtAV.Text = string.Empty  ; 
				combCYIsHaveKQTP.Enabled = true ; //抽油含醛	
				combT8WIsHaveKQTP.Enabled = true ; //塔8水含醛	
				txtASTM.ReadOnly = false ;//色度
				txtASTM.Text = string.Empty  ; 
				combGetSampPersonTP.Enabled = true ; //采样人	
				combTestPersonTP.Enabled = true ; //检测人	
				combGetSampLocationTP.Enabled = true ; //采样地点	
		...
		private void SetControlEdit()
		{
			if (CutShowMode== FormShowMode.ContinueDisplay || CutShowMode== FormShowMode.DisplayCurrent ) {
				...
				btnOK.Text =" 保存";
				//控件除主键外都可读
				txtID.ReadOnly = true;//数据编号combRawNameTP.Enabled = true ;//原料名称
				combJQIsHaveKQTP.Enabled = true ;//精油含醛
				txtAV.ReadOnly = false ;//酸值
				combCYIsHaveKQTP.Enabled = true ;//抽油含醛
				combT8WIsHaveKQTP.Enabled = true ;//塔8水含醛
		...
		private void SetControlReadOnly()
		{
			...
			combRawNameTP.Enabled = false ;//原料名称
			combJQIsHaveKQTP.Enabled = false ;//精油含醛
			txtAV.ReadOnly = true  ;//酸值
			combCYIsHaveKQTP.Enabled = false ;//抽油含醛
			combT8WIsHaveKQTP.Enabled = false ;//塔8水含醛
			...
			combGetSampLocationTP.Enabled = false ;//采样地点
		...
		private void BandingData()
		{
			...
			combRawNameTP.DataBindings.Clear () ;//原料名称
			combRawNameTP.DataBindings.Add ("Text",CutModel,"RawNameTP") ;
			combJQIsHaveKQTP.DataBindings.Clear () ;//精油含醛
			...
			combCYIsHaveKQTP.DataBindings.Clear () ;//抽油含醛
			combCYIsHaveKQTP.DataBindings.Add ("Text",CutModel,"CYIsHaveKQTP") ;
			combT8WIsHaveKQTP.DataBindings.Clear () ;//塔8水含醛
			...
			combGetSampLocationTP.DataBindings.Clear () ;//采样地点
