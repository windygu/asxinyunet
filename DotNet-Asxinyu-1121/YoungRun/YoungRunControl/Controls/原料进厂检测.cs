...
private System.Windows.Forms.TextBox txtRemark ;
   ...
   private System.Windows.Forms.ErrorProvider errorProvider1 ;
		...
		#region 构造函数 及初始化
		public AddRawIntoFactory()	{InitializeComponent(); CustomerSettings(); }	
		...
		/// 获取或者设置当前实体
		/// </summary>
		public tb_RawIntoFactory CutModel{get ;set ;}
		/// <summary>
		/// 当前实体窗体的显示模式,默认为连续显示
		/// </summary>
		...
		bool ValidateControls()
		{
			...
			if(combProviderTP.Text.Trim()=="")//供应方
			{
		...
		void btnOK_Click(object sender, EventArgs e)
		{
			...
			else 
			{
				if(((CutShowMode!= FormShowMode.ReadOnlyForAll) || (CutShowMode != FormShowMode.ReadOnlyForOne)) && ValidateControls() )
				{
					...
					if(combProviderTP.Text.Trim()!="") model.ProviderTP = combProviderTP.Text.Trim() ;//供应方
		...
		List <tb_RawIntoFactory> modelList ;//当前实体列表
		void GetData()
		{				
			//判断不为空，才能绑定				
		...
		private void SetControlReSet()
		{
			...
			if (CutShowMode == FormShowMode.AddOne || CutShowMode== FormShowMode.ContinueAdd )
			{
				...
				combProviderTP.Enabled = true ; //供应方	
		...
		private void SetControlEdit()
		{
			if (CutShowMode== FormShowMode.ContinueDisplay || CutShowMode== FormShowMode.DisplayCurrent ) {
				...
				btnOK.Text =" 保存";
				//控件除主键外都可读
				txtID.ReadOnly = true;//记录编号combOilNameTP.Enabled = true ;//油品名称
				combProviderTP.Enabled = true ;//供应方
				txtDataID.ReadOnly = false ;//数据编号
				combSendPersonTP.Enabled = true ;//送样人
		...
		private void SetControlReadOnly()
		{
			...
			combProviderTP.Enabled = false ;//供应方
		...
		private void BandingData()
		{
			...
			combProviderTP.DataBindings.Clear () ;//供应方
