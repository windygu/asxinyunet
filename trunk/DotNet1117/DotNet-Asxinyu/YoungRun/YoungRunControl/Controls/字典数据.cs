...
private System.Windows.Forms.TextBox txtRemark ;
   ...
   private System.Windows.Forms.ErrorProvider errorProvider1 ;
		...
		#region 构造函数 及初始化
		public AddDicType()	{InitializeComponent(); CustomerSettings(); }	
		...
		/// 获取或者设置当前实体
		/// </summary>
		public tb_DicType CutModel{get ;set ;}
		/// <summary>
		/// 当前实体窗体的显示模式,默认为连续显示
		/// </summary>
		...
		bool ValidateControls()
		{
			...
			if(combTypeName.Text.Trim()=="")//类型名称
			{
				errorProvider1.SetError(combTypeName,"必填项目");
				return false ;
			} 
			if(txtValue.Text.Trim()=="")//数据值
			{
		...
		void btnOK_Click(object sender, EventArgs e)
		{
			...
			else 
			{
				if(((CutShowMode!= FormShowMode.ReadOnlyForAll) || (CutShowMode != FormShowMode.ReadOnlyForOne)) && ValidateControls() )
				{
					...
					if(combTypeName.Text.Trim()!="") model.TypeName = combTypeName.Text.Trim() ;//类型名称
					if(txtValue.Text.Trim()!="")  model.Value = txtValue.Text.Trim()  ;//数据值
		...
		List <tb_DicType> modelList ;//当前实体列表
		void GetData()
		{				
			//判断不为空，才能绑定				
		...
		private void SetControlReSet()
		{
			...
			if (CutShowMode == FormShowMode.AddOne || CutShowMode== FormShowMode.ContinueAdd )
			{
				...
				combTypeName.Enabled = true ; //类型名称	
				txtValue.ReadOnly = false ;//数据值
		...
		private void SetControlEdit()
		{
			if (CutShowMode== FormShowMode.ContinueDisplay || CutShowMode== FormShowMode.DisplayCurrent ) {
				...
				btnOK.Text =" 保存";
				//控件除主键外都可读
				txtID.ReadOnly = true;//编号combTypeName.Enabled = true ;//类型名称
				txtValue.ReadOnly = false ;//数据值
				txtRemark.ReadOnly = false ;//备注
				}
		}
		private void SetControlReadOnly()
		{
			...
			combTypeName.Enabled = false ;//类型名称
			txtValue.ReadOnly = true  ;//数据值
		...
		private void BandingData()
		{
			...
			combTypeName.DataBindings.Clear () ;//类型名称
			combTypeName.DataBindings.Add ("Text",CutModel,"TypeName") ;
			txtValue.DataBindings.Clear();//数据值
