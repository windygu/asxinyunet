...
namespace ProteidCalculate
{
    ...
    public partial class ProteidCharacter
    {
        ...
        public static string[] GetCentrlString(string seqData, int strLength,char desChar)
        {
            //首先进行转换
        ...
        /// <returns></returns>
        public static string[] GetCentrlString(string seqData, int strLength, char desChar,out int[] pos)
        {
            //首先进行转换
            seqData = seqData.ToUpper();
            //检测是否包含其他非规定字符：B、J、O、U、X,并剔除
        ...
        public static double[][] GetAllWaccAndACFvalue(string text, int strLength, char desChar)
        {
            ...
            for (int i = 0; i < allTextStr.Length; i++)
            {
                ...
                {
                    tempDouble = GetOneWaccAndACFvalue(tempStr[j ], 2 * strLength + 1);//计算联合特征值：前20列为Wacc权重特征值,后面的列为自相关特征
       ...
       //获取一条序列的正规重量
        ...
        #region 使用的特征提取是自相关函数和分组重量和氨基酸位置权重  2
        /// <summary>
        /// 自相关函数和分组重量和氨基酸位置权重
        /// </summary>
        /// <param name="text">所有输入的文本块</param>
        ...
        public static double[][] Ace_Pred(string text, int strLength, char desChar,int length)
        {
        	...
        	for (int i = 0 ; i <allText.Length ; i ++)
        	{
        		...
        		{
        			//计算结果
        			double[] temp1 = new double[20+2*strLength +1+length*3 ] ;
        			double[] temp2,temp3 ;
        ...
        public static double[][] PredSulSite(string text, int strLength, char desChar,int length)
        {
        	...
        	for (int i = 0 ; i <allText.Length ; i ++)
        	{
        		...
        		{
        			//计算结果
        			double[] temp1 = new double[2*strLength +1+length*3 ] ;
        			double[] temp2,temp3 ;
