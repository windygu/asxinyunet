...
namespace SVM
{
    ...
    public static class Prediction
    {
            ...
            bool predict_probability)
        {
            ...
            for (int i = 0; i < problem.Count; i++)
            {
                ...
                if (predict_probability && (svm_type == SvmType.C_SVC || svm_type == SvmType.NU_SVC))
                {
                    ...
                    if (i ==1) //测试代码
                    {
                        Console.WriteLine(prob_estimates[1]);
       ...
       /// <summary>
       ///  重载修改方法,predict_probability为true的时候
       /// </summary>
       /// <param name="problem"></param>
       /// <param name="model"></param>
       /// <param name="probaResult"></param>
       /// <returns></returns>
        public static double Predict(Problem problem, Model model,out double[] probaResult)
        {
            int correct = 0;
            int total = 0;
            double error = 0;
            ...
            int[] labels = new int[nr_class];
            double[] prob_estimates = null;
            bool predict_probability = true ;
            probaResult = new double[problem.Count ] ;//输出的结果
            if (predict_probability)
            {
                if (svm_type == SvmType.EPSILON_SVR || svm_type == SvmType.NU_SVR)
                {
            ...
            for (int i = 0; i < problem.Count; i++)
            {
                ...
                {
                    v = Procedures.svm_predict_probability(model, x, prob_estimates);
                    probaResult [i ] = prob_estimates[0] ;                   
                }
                else
                {
                    v = Procedures.svm_predict(model, x);                   
