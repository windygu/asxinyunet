	...
	namespace YoungRunControl.Controls
	{
			...
			//TODO:有问题，需要根据当前的状态来更新和保存数据
				...
				if(combTankIdTP.Text.Trim()!=""){
					model.TankIdTP = combTankIdTP.Text.Trim() ;}
				if(combProductNameTP.Text.Trim()!=""){
					model.ProductNameTP = combProductNameTP.Text.Trim() ;}if(txtLiquidLevel.Text.Trim()!=""){
						model.LiquidLevel =Convert.ToDouble(txtLiquidLevel.Text.Trim()) ;}
					if(txtCurVolume.Text.Trim()!=""){
						model.CurVolume =Convert.ToDouble(txtCurVolume.Text.Trim()) ;}
					if(txtCurWeigth.Text.Trim()!=""){
						model.CurWeigth =Convert.ToDouble(txtCurWeigth.Text.Trim()) ;}
					if(txtD20.Text.Trim()!=""){
						model.D20 =Convert.ToDouble(txtD20.Text.Trim()) ;}
					if(txtCurTemperature.Text.Trim()!=""){
						model.CurTemperature =Convert.ToDouble(txtCurTemperature.Text.Trim()) ;}
					{
						model.RecordTime = dtRecordTime.Value ;}{
			...
			private void SetControlReSet()
			{
				...
				if (CutShowMode == FormShowMode.AddOne || CutShowMode== FormShowMode.ContinueAdd )
				{
					...
					txtID.ReadOnly = false ;
							...
							combTankIdTP.Enabled = true ;
							combProductNameTP.Enabled = true ;txtLiquidLevel.ReadOnly = false ;
							txtLiquidLevel.Text = string.Empty  ; txtCurVolume.ReadOnly = false ;
							txtCurVolume.Text = string.Empty  ; txtCurWeigth.ReadOnly = false ;
							txtCurWeigth.Text = string.Empty  ; txtD20.ReadOnly = false ;
							txtD20.Text = string.Empty  ; txtCurTemperature.ReadOnly = false ;
							txtCurTemperature.Text = string.Empty  ; txtRemark.ReadOnly = false ;
			...
			private void SetControlEdit()
			{
				if (CutShowMode== FormShowMode.ContinueDisplay || CutShowMode== FormShowMode.DisplayCurrent ) {
					...
					btnOK.Text =" 保存";
					//控件除主键外都可读
				txtID.ReadOnly = true;combTankIdTP.Enabled = true ;combProductNameTP.Enabled = true ;
					txtLiquidLevel.ReadOnly = false ;
					txtCurVolume.ReadOnly = false ;
					txtCurWeigth.ReadOnly = false ;
					txtD20.ReadOnly = false ;
					txtCurTemperature.ReadOnly = false ;dtRecordTime.Enabled = true ;
				dtUpdateTime.Enabled = true ;
			...
			private void SetControlReadOnly()
			{
				...
				txtID.ReadOnly = true ;
							combTankIdTP.Enabled = false ;
							
							combProductNameTP.Enabled = false ;
							txtLiquidLevel.ReadOnly = true ;txtCurVolume.ReadOnly = true ;txtCurWeigth.ReadOnly = true ;txtD20.ReadOnly = true ;txtCurTemperature.ReadOnly = true ;
							dtRecordTime.Enabled = false ;
			...
			private void BandingData()
			{
				  ...
				  combTankIdTP.DataBindings.Clear () ;
