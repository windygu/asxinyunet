...
{
	public partial class OilSampleTestManage:UserControl
	{
		#region 初始化
		public OilSampleTestManage()
		{
			InitializeComponent();
			string maininfo = "油样检测数据表" ;
		...
		void GetData()
		{
			winPage.RecordCount =tb_oilsampletest.FindCount(cutSql ,"","",0,0) ;
			btList = tb_oilsampletest.FindAll(cutSql, "", "", (winPage.PageIndex -1)*winPage.PageSize ,
			                                  winPage.PageSize );
		...
		{
			WinFormHelper.DeleteDgvItems<tb_oilsampletest>(sender, e, dgv, btList);
		...
		{
			AddOilSampleTestForm ast = new AddOilSampleTestForm () ;
		...
		void ToolExportToExcelClick(object sender, EventArgs e)
		{
            ...
            sf.EntityName = typeof(tb_oilsampletest );
