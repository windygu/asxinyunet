...
namespace DotNet.Business
{
    ...
    public partial class BasePermissionScopeManager : BaseManager, IBaseManager
    {
        ...
        #region public DataTable GetOrganizeDT(string managerUserId, string permissionItemCode) 按某个权限获取组织机构 数据表
        /// <summary>
        /// 按某个权限获取组织机构 数据表
        /// </summary>
        /// <param name="managerUserId">管理用户主键</param>
        ...
        public DataTable GetOrganizeDT(string managerUserId, string permissionItemCode)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //  string[] ids = this.GetTreeResourceScopeIds(managerUserId, BaseOrganizeEntity.TableName, permissionItemCode, true);
                    string[] ids = this.GetTreeResourceScopeIds(managerUserId, BaseOrganizeEntity.TableName, permissionItemCode, false);
                    sqlQuery = StringUtil.ArrayToList(ids);
                }
            }
            ...
            sqlQuery = " SELECT * FROM " + BaseOrganizeEntity.TableName
                     + " WHERE " + BaseOrganizeEntity.TableName + "." + BaseOrganizeEntity.FieldId + " IN (" + sqlQuery + ") "
        ...
        public DataTable GetRoleDT(string userId, string permissionItemCode)
        {
            ...
            // 用户管理员,这里需要判断,是业务权限？
            isRole = userRoleManager.UserIsInRoleByCode(userId, "UserAdmin") || userRoleManager.UserIsInRoleByCode(userId, "Admin");
            if (isRole)
            {
        ...
        public string[] GetResourceScopeIds(string userId, string targetCategory, string permissionItemCode)
        {
            ...
            sqlQuery =  
                        // 用户的权限
                          " SELECT BasePermissionScope.TargetId "
                        + "   FROM BasePermissionScope "
                        + "  WHERE (BasePermissionScope.ResourceCategory = '" + BaseUserEntity.TableName + "') "
                        ...
                        + "                  AND (BaseUserRole.DeletionStateCode = 0) ) ";
                        if (!string.IsNullOrEmpty(defaultRoleId))
                        {
                            // 用户的默认角色
                            sqlQuery += " OR (BasePermissionScope.ResourceId = '" + defaultRoleId + "')";
                        }
                        sqlQuery += " ) " 
                        + " ) ";
