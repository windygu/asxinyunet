...
namespace DotNet.Business
{
    ...
    public class WorkFlowProcessAdminService : System.MarshalByRefObject, IWorkFlowProcessAdminService
    {
        ...
        #region public DataTable GetDataTable(BaseUserInfo BaseUserInfo) 获取数据
        /// <summary>
        /// 获取数据
        /// </summary>
        ...
        public DataTable GetBillTemplateDT(BaseUserInfo userInfo)
        {
            ...
            DataTable dataTable = new DataTable(WorkFlowBillTemplateManager.TemplateTable);
            using (IDbHelper dbHelper = DbHelperFactory.GetHelper(BaseSystemInfo.WorkFlowDbType))
            {
                try
                {
                    dbHelper.Open(WorkFlowDbConnection);
                    WorkFlowBillTemplateManager templateManager = new WorkFlowBillTemplateManager(dbHelper, userInfo);
                    dataTable = templateManager.GetDataTable(new KeyValuePair<string, object>(BaseNewsEntity.FieldDeletionStateCode, 0), BaseNewsEntity.FieldSortCode);
                    BaseLogManager.Instance.Add(dbHelper, userInfo, this.serviceName, MethodBase.GetCurrentMethod());
                }
                catch (Exception ex)
                {
