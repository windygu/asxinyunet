...
namespace DotNet.Business
{
    ...
    public abstract partial class BaseManager : IBaseManager
    {
        ...
        public virtual int SetTableColumns(string tableCode, string tableName, string columnCode, string columnName)
        {
            ...
            string sortCode = sequenceManager.GetSequence(BaseTableColumnsEntity.TableName);

            // 插入数据
            SQLBuilder sqlBuilder = new SQLBuilder(DbHelper);
            sqlBuilder.BeginInsert(BaseTableColumnsEntity.TableName);

            sqlBuilder.SetValue(BaseTableColumnsEntity.FieldTableCode, tableCode);
            sqlBuilder.SetValue(BaseTableColumnsEntity.FieldColumnCode, columnCode);
            sqlBuilder.SetValue(BaseTableColumnsEntity.FieldColumnName, columnName);
            sqlBuilder.SetValue(BaseTableColumnsEntity.FieldSortCode, sortCode);
