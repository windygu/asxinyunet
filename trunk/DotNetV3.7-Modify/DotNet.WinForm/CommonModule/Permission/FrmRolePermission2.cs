    ...
    public partial class FrmRolePermission2 : BaseForm
    {
        ...
        private void tvModule_AfterCheck(object sender, TreeViewEventArgs e)
        {
            if (this.isUserClick)
            {
                ...
                {
                    // 授予操作权限
                    DotNetService.Instance.PermissionService.GrantRoleModuleScope(this.UserInfo, this.TargetRoleId, "Resource.AccessPermission", e.Node.Tag.ToString());
                }
                else
                {
                    // 撤销操作权限
                    DotNetService.Instance.PermissionService.RevokeRoleModuleScope(this.UserInfo, this.TargetRoleId, "Resource.AccessPermission", e.Node.Tag.ToString());
                }
        ...
        private void tvPermissionItem_AfterCheck(object sender, TreeViewEventArgs e)
        {
            if (this.isUserClick)
            {
                ...
                {
                    // 授予操作权限
                    DotNetService.Instance.PermissionService.GrantRolePermissionById(this.UserInfo, this.TargetRoleId, e.Node.Tag.ToString());
                }
                else
                {
                    // 撤销操作权限
                    DotNetService.Instance.PermissionService.RevokeRolePermissionById(this.UserInfo, this.TargetRoleId, e.Node.Tag.ToString());
                }
                for (int i = 0; i < e.Node.Nodes.Count; i++)
                {
                    e.Node.Nodes[i].Checked = e.Node.Checked;
