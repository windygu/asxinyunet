    ...
    public partial class FrmUserPermissionBatchSet : BaseForm
    {
        ...
        private void tvModule_AfterCheck(object sender, TreeViewEventArgs e)
        {
            if (this.isUserClick)
            {
                if (e.Node.Checked)
                {
                    ...
                    DotNetService.Instance.PermissionService.GrantUserModuleScope(this.UserInfo, this.TargetUserId, "Resource.AccessPermission", e.Node.Tag.ToString());
                }
                else
                {
                    // 撤销操作权限
                    DotNetService.Instance.PermissionService.RevokeUserModuleScope(this.UserInfo, this.TargetUserId, "Resource.AccessPermission", e.Node.Tag.ToString());
                }
            }
        }
        ...
        private void tvPermissionItem_AfterCheck(object sender, TreeViewEventArgs e)
        {
            if (this.isUserClick)
            {
                ...
                {
                    // 授予操作权限
                    DotNetService.Instance.PermissionService.GrantUserPermissionById(this.UserInfo, this.TargetUserId, e.Node.Tag.ToString());
                }
                else
                {
                    // 撤销操作权限
                    DotNetService.Instance.PermissionService.RevokeUserPermissionById(this.UserInfo, this.TargetUserId, e.Node.Tag.ToString());
                }
            }
        }
