...
namespace DotNet.WinForm
{
    ...
    public partial class UCAutoWorkFlowStatr : UserControl
    {
        ...
        private bool SendTo()
        {
            ...
            if (!string.IsNullOrEmpty(this.ObjectId) && !string.IsNullOrEmpty(this.CategoryId))
            {
                string returnStatusCode = string.Empty;
                // DotNetService.Instance.WorkFlowCurrentService.AutoStatr(this.UserInfo, this.CategoryId, this.CategoryFullName, new string[] { this.ObjectId }, this.ObjectFullName, this.WorkFlowCode, this.txtAuditIdea.Text, out returnStatusCode);
        ...
        private void btnSendTo_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!this.BeforBtnSendToClick(this.CategoryId, this.ObjectId))
                {
                    return;
                }
            }
            if (this.CheckInput())
            {
                ...
                if (this.SendTo())
                {
                    ...
                    {
                        this.AfterBtnSendToClick(this.CategoryId, this.ObjectId);
                    }
                }
            }
        }
