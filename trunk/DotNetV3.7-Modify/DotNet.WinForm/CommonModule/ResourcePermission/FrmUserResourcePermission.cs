...
namespace DotNet.WinForm
{
    ...
    public partial class FrmUserResourcePermission : BaseForm
    {
        ...
        #region private void GetUserList() 获取角色列表
        /// <summary>
        /// 获取角色列表
        /// </summary>
        ...
        private void GetUserPermission()
        {
            ...
            // 被选中的权限
            this.setSelectIds = DotNetService.Instance.PermissionService.GetPermissionScopeTargetIds(this.UserInfo, BaseUserEntity.TableName, this.TargetUserId, this.TargetResourceCategory, this.PermissionItemCode);
            // 读取资源
            // 读取资源
            ...
            // 往DataTable中添加选择列
            //BaseInterfaceLogic.DataTableAddColumn(this.DTTargetResource, "colSelected");
            // 检查选中状态
        ...
        private int BatchSave()
        {
            ...
            if (grantResourceIds.Length > 0)
            {
                returnValue += DotNetService.Instance.PermissionService.GrantPermissionScopeTargets(this.UserInfo, BaseUserEntity.TableName, this.TargetUserId, this.TargetResourceCategory, grantResourceIds, this.PermissionItemId);
            }
            // 被取消的权限
            string[] revokeResourceIds = this.GetUnSelectedIds();
            if (revokeResourceIds.Length > 0)
            {
                returnValue += DotNetService.Instance.PermissionService.RevokePermissionScopeTargets(this.UserInfo, BaseUserEntity.TableName, this.TargetUserId, this.TargetResourceCategory, revokeResourceIds, this.PermissionItemId);
            }
            return returnValue;
        }
        #endregion
