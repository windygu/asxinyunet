...
namespace DotNet.WinForm
{
    ...
    /// </author> 
    /// </summary>
    public partial class FrmPermissionSelect : BaseForm, IBaseSelectPermissionForm
    {
        ...
        private void GetSelecteTreeNodes(TreeNode treeNode)
        {
            if ((treeNode != null) && (treeNode.Tag != null))
            {
                if (treeNode.Checked)
                {
                    this.SelecteTreeNodesId += treeNode.Tag.ToString() + ",";
                }
                for (int i = 0; i < treeNode.Nodes.Count; i++)
                {
                    // 这里进行递规操作
        ...
        {
            // 当前选择的权限
            this.SelectedId = this.tvPermission.SelectedNode.Tag.ToString();
            this.SelectedFullName = this.tvPermission.SelectedNode.Text;
            // 检查移动的有效性
            if (this.CheckMove)
            {
