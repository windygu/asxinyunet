...
namespace DotNet.WinForm
{
    ...
    public partial class FrmPermissionItemAdmin : BaseForm, IBaseForm
    {
        ...
        public string ParentEntityId
        {
            get
            {
                ...
                {
                    this.parentEntityId = this.tvPermission.SelectedNode.Tag.ToString();
                }
                else
                {
   ...
   public string CurrentEntityId
        ...
        private void tvPermission_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (e.Data.GetDataPresent("System.Windows.Forms.TreeNode", false))
            {
                ...
                if (BaseInterfaceLogic.TreeNodeCanMoveTo(treeNode, targetTreeNode))
                {
                    ...
                    DotNetService.Instance.PermissionItemService.MoveTo(UserInfo, treeNode.Tag.ToString(), targetTreeNode.Tag.ToString());
                    // 往目标节点中加入被拖动节点的一份克隆
        ...
        {
            PermissionList += treeNode.Tag.ToString() + ",";
        ...
        {
            // 全部导出Excel
            DataTable dataTable = DotNetService.Instance.PermissionItemService.GetDataTable(UserInfo);
            dataTable.DefaultView.Sort = BasePermissionItemEntity.FieldParentId + ", " + BasePermissionItemEntity.FieldSortCode;
            string exportFileName = this.Text + ".csv";
            this.ExportExcel(this.grdPermission, dataTable.DefaultView, @"\Modules\Export\", exportFileName);
        }
        ...
        private void EditTree()
        {
            ...
            FrmPermissionItemEdit frmPermissionItemEdit = new FrmPermissionItemEdit(this.tvPermission.SelectedNode.Tag.ToString());
            if (frmPermissionItemEdit.ShowDialog(this) == DialogResult.OK)
            {
                // 编辑节点
        ...
        public int SingleDelete()
        {
            ...
            else
            {
                if (MessageBox.Show(AppMessage.MSG0015, AppMessage.MSG0000, MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.OK)
                {
                    ...
                    if (returnValue > 0)
                    {
                        string[] tags = {this.tvPermission.SelectedNode.Tag.ToString()};
                        BaseInterfaceLogic.BatchRemoveNode(this.tvPermission, tags);
                        // 绑定grdPermission
                        this.GetPermissionList();
                    }
