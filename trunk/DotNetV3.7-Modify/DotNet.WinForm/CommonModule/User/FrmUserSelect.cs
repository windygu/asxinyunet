...
namespace DotNet.WinForm
{
    ...
    /// </author> 
    /// </summary>
    public partial class FrmUserSelect : BaseForm, IBaseSelectUserForm
    {
        ...
        //    // 设置表主键
        //    DataColumn[] primaryKey = new DataColumn[] { this.DTUser.Columns[BaseUserEntity.FieldId] };
        ...
        private void FormOnLoad(bool loadUser, string searchValue)
        {
            if (loadUser)
            {
                ...
                // 检查是否需要移出
                if (this.RemoveIds != null && this.RemoveIds.Length > 0)
                {
                    for (int i = 0; i < this.RemoveIds.Length; i++)
                    {
                        DataRow dataRow = this.DTUser.Rows.Find(RemoveIds[i]);
                        if (dataRow != null)
                        {
                            dataRow.Delete();
                        }
