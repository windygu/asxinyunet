...
namespace DotNet.WinForm
{
    ...
    public partial class FrmRoleUserAdmin : BaseForm
    {
        ...
        public FrmRoleUserAdmin(string roleId) : this()
        {
            this.TargetRoleId = roleId;
        }

        public FrmRoleUserAdmin(string roleId, string roleName) : this()
        {
            this.TargetRoleId = roleId;
        ...
        private void BindData()
        {
            ...
            // {
            //    bool selected = Array.IndexOf(this.UserIds, dataRowView.Row[BaseUserEntity.FieldId].ToString()) >= 0;
            //    dataRowView.Row[BaseBusinessLogic.SelectedColumn] = selected.ToString();
            // }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            ((IBaseSelectUserForm)frmUserSelect).AllowNull = false;
            ((IBaseSelectUserForm)frmUserSelect).MultiSelect = true;
            ((IBaseSelectUserForm)frmUserSelect).PermissionItemScopeCode = this.PermissionItemScopeCode;
            ((IBaseSelectUserForm)frmUserSelect).RemoveIds = this.GetIds();
            if (frmUserSelect.ShowDialog() == DialogResult.OK)
            {
                DotNetService.Instance.RoleService.AddUserToRole(UserInfo, this.TargetRoleId, ((IBaseSelectUserForm)frmUserSelect).SelectedIds);
                // 加载窗体
                this.FormOnLoad();
                // 设置按钮状态
                this.SetControlState();
