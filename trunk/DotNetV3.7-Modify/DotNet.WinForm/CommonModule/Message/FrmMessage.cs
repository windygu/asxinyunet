...
namespace DotNet.WinForm
{
    ...
    public partial class FrmMessage : BaseForm
    {
        ...
        private void LoadTreeNodes(DataTable dataTable, string fieldId, string fieldParentId, string fieldFullName, TreeView treeView, TreeNode treeNode, bool loadTree = true)
        {
            ...
            {
                dataRows = dataTable.Select(fieldParentId + "=" + treeNode.Tag.ToString());
            }
            foreach (DataRow dataRow in dataRows)
            {
                // 节点不为空，并且是当前节点的子节点
                if ((treeNode.Tag != null) && !(treeNode.Tag.ToString().Equals(dataRow[fieldParentId].ToString())))
                {
                    continue;
                }
                // 当前节点的子节点, 加载根节点
                if (dataRow.IsNull(fieldParentId)
                    ...
                    || ((treeNode.Tag != null) && (treeNode.Tag.ToString().Equals(dataRow[fieldParentId].ToString()))))
                {
        ...
        FrmMessageRemind frmMessageRemind = new FrmMessageRemind();
            ...
            if (!String.IsNullOrEmpty(returnValue[0]))
            {
                ...
                if (!String.IsNullOrEmpty(returnValue[1]))
                {
                    ...
                    if (!lastChekDate.Equals(this.LastChekDate))
                    {
                        ...
                        if (!this.frmMessageRemind.Visible)
                        {
                            this.frmMessageRemind = new FrmMessageRemind(returnValue[4], returnValue[2], returnValue[3], returnValue[5]);
                        }
                        else
                        {
                            this.frmMessageRemind.SetRemind(returnValue[4], returnValue[2], returnValue[3], returnValue[5]);
                        }
        ...
        private void GetUserList(TreeNode selectedNode)
        {
            ...
            try
            {
                ...
                foreach (DataRow dataRow in this.DTUser.Rows)
                {
                    ...
                    {
                        TreeNode treeNode = new TreeNode();
                        treeNode.Tag = dataRow[BaseUserEntity.FieldId].ToString();
        ...
        public FrmMessageRead ShowMessageRead(string receiverId, string receiverRealName, IWin32Window owner)
        {
            return this.ShowMessageRead(receiverId, receiverRealName, owner, FormStartPosition.WindowsDefaultLocation);
        }

        public FrmMessageRead ShowMessageRead(string receiverId, string receiverRealName, IWin32Window owner, FormStartPosition formStartPosition)
        {
            FrmMessageRead frmMessageRead = null;
            if (!this.ReceiveUserList.Contains(receiverId))
            {
                this.ReceiveUserList.Add(receiverId);
                frmMessageRead = new FrmMessageRead(receiverId, receiverRealName);
        ...
        public FrmMessageRead ShowMessageRead(string receiverId, string receiverFullName)
        {
            return this.ShowMessageRead(receiverId, receiverFullName, this);
        }
        ...
        public void ShowMessage(BaseMessageEntity messageEntity)
        {
            ...
            {
                // 检查窗体，是否已经打开了窗体
                FrmMessageRead frmMessageRead = this.ShowMessageRead(messageEntity.CreateUserId, messageEntity.CreateBy, this);
        ...
        private void tvTree_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (treeView.SelectedNode != null)
            {
                if (treeView.SelectedNode.Tag != null)
                {
                    ...
                    {
                        // 不允许自己跟自己通讯
                        if (!UserInfo.Id.Equals(treeView.SelectedNode.Tag.ToString()))
                        {
                            this.ShowMessageRead(treeView.SelectedNode.Tag.ToString(), treeView.SelectedNode.Text, this, FormStartPosition.CenterScreen);
                        }
                    }
                }
            }
            ...
            if (e.Data.GetDataPresent("System.Windows.Forms.TreeNode", false))
            {
                ...
                if (targetTreeNode.SelectedImageIndex <= 2)
                {
                    ...
                    if (BaseInterfaceLogic.TreeNodeCanMoveTo(treeNode, targetTreeNode))
                    {
                        ...
                        this.MoveUserTo(treeNode.Tag.ToString(), targetTreeNode);
                        // 往目标节点中加入被拖动节点的一份克隆
        ...
        private void SetTreeViewOnLineState(DataTable dataTable, TreeView treeView)
        {
            ...
            for (int i = 0; i < treeView.Nodes.Count; i++)
            {
                ...
                while (treeNode != null)
                {
                    ...
                    {
                        state = "0";
                        DataRow[] dataRow = dataTable.Select(BaseUserEntity.FieldId + "='" + treeNode.Tag.ToString() + "'");
                        for (int j = 0; j < dataRow.Length; j++)
                        {
                            if (dataRow[j][BaseUserEntity.FieldId].ToString() == treeNode.Tag.ToString())
                            {
        ...
        private void mitmFrmMessageSend_Click(object sender, EventArgs e)
        {
            ...
            if (treeView.SelectedNode != null)
            {
                ...
                frmMessageSend.SetSendTo(treeView.SelectedNode.Tag.ToString(), treeView.SelectedNode.Text);
            }
        }
        ...
        private void mitmFrmWorkReportAdd_Click(object sender, EventArgs e)
        {
            if (this.tvOrganize.SelectedNode != null)
            {
                ...
                Form frmWorkReportAdd = (Form)Activator.CreateInstance(type, this.tvOrganize.SelectedNode.Tag.ToString(), this.tvOrganize.SelectedNode.Text);
        ...
        private void mitmFrmWorkReportList_Click(object sender, EventArgs e)
        {
            if (this.tvOrganize.SelectedNode != null)
            {
                ...
                Type type = CacheManager.Instance.GetType(assemblyName, formName);
                Form frmWorkReportList = (Form)Activator.CreateInstance(type, this.tvOrganize.SelectedNode.Tag.ToString(), this.tvOrganize.SelectedNode.Text);
        ...
        private void mitmFrmUserPermission_Click(object sender, EventArgs e)
        {
            ...
            if (treeView.SelectedNode != null)
            {
                ...
                Type type = CacheManager.Instance.GetType(assemblyName, formName);
                Form frmUserPermissionAdmin = (Form)Activator.CreateInstance(type, treeView.SelectedNode.Tag.ToString(), treeView.SelectedNode.Text);
        ...
        private void mitmFrmUserEdit_Click(object sender, EventArgs e)
        {
            ...
            if (treeView.SelectedNode != null)
            {
                ...
                Type type = CacheManager.Instance.GetType(assemblyName, formName);
                Form frmStaffEdit = (Form)Activator.CreateInstance(type, treeView.SelectedNode.Tag.ToString());
        ...
        private void mitmFrmUserRoleAdmin_Click(object sender, EventArgs e)
        {
            ...
            if (treeView.SelectedNode != null)
            {
                ...
                Type type = CacheManager.Instance.GetType(assemblyName, formName);
                Form frmRoleUserAdmin = (Form)Activator.CreateInstance(type, treeView.SelectedNode.Tag.ToString(), treeView.SelectedNode.Text);
        ...
        private void mitmFrmStaffAddressEdit_Click(object sender, EventArgs e)
        {
            ...
            if (treeView.SelectedNode != null)
            {
                ...
                Type type = CacheManager.Instance.GetType(assemblyName, formName);
                Form frmStaffAddressEdit = (Form)Activator.CreateInstance(type, treeView.SelectedNode.Tag.ToString());
        ...
        private void mitmFrmLogByUser_Click(object sender, EventArgs e)
        {
            ...
            if (treeView.SelectedNode != null)
            {
                ...
                Type type = CacheManager.Instance.GetType(assemblyName, formName);
                Form frmLogByUser = (Form)Activator.CreateInstance(type, treeView.SelectedNode.Tag.ToString(), treeView.SelectedNode.Text);
