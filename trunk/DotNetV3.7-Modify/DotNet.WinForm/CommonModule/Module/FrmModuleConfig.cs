    ...
    public partial class FrmModuleConfig : BaseForm
    {
        ...
        public string CurrentEntityId
        {
            get
            {
                ...
                {
                    this.currentEntityId = this.tvModule.SelectedNode.Tag.ToString();
                }
        ...
        private void LoadTreeModule(TreeNode treeNode)
        {
            ...
            {
                // 判断不为空的当前节点的子节点
                if ((treeNode.Tag != null) && (treeNode.Tag.ToString().Length > 0) && (!treeNode.Tag.Equals(dataRow[BaseModuleEntity.FieldParentId].ToString())))
                {
                    continue;
                }
                // 当前节点的子节点, 加载根节点
                if ((dataRow.IsNull(BaseModuleEntity.FieldParentId) || (dataRow[BaseModuleEntity.FieldParentId].ToString().Length == 0) || (dataRow[BaseModuleEntity.FieldParentId].ToString().Equals("0")) || (dataRow[BaseModuleEntity.FieldParentId].ToString().Equals(BaseSystemInfo.RootMenuCode)) || ((treeNode.Tag != null) && treeNode.Tag.Equals(dataRow[BaseModuleEntity.FieldParentId].ToString()))))
                {
        ...
        {
            // 提高运行速度
            DataRow[] dataRows = this.DTModule.Select(BaseModuleEntity.FieldId + "='" + treeNode.Tag.ToString() + "'");
            foreach (DataRow dataRow in dataRows)
            {
                dataRow[BaseModuleEntity.FieldEnabled] = treeNode.Checked ? 1 : 0;
            }
            // BUBaseBusinessLogic.SetProperty(this.DSModule.Tables[BaseModuleEntity.TableName], TreeNode.Tag.ToString(), BaseModuleEntity.FieldEnabled, TreeNode.Checked ? 1 : 0);
            for (int i = 0; i < treeNode.Nodes.Count; i++)
            {
                // 这里进行递规操作
