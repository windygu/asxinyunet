        ...
        private string GetEditEntityValue(string field, string dataType)
        {
            ...
            else
            {
                ...
                switch (dataType.ToUpper())
                {
                    ...
                    default:
                        returnValue += System.Environment.NewLine;
                        returnValue += "        entity." + field + " = this.txt" + field + ".Text;" + System.Environment.NewLine;
                        break;
                }
            }
            return returnValue;
                ...
                if (((XmlNode)xmlNode.ChildNodes[i]).LocalName.Equals("Columns"))
                {
                    for (int j = 0; j < xmlNode.ChildNodes[i].ChildNodes.Count; j++)
                    {
                        ...
                        if (!field.EndsWith("Id"))
                        {
                            ...
                            {
                                // 上面的写法.NET早期的版本里会有错误
                                // showEntity = System.Environment.NewLine + "		this.cmb" + field + ".SelectedValue = " + "entity." + field + ";";
                                ...
                                itemDetails += System.Environment.NewLine
                                            +"        BaseItemDetailsManager manager" + field + " = new BaseItemDetailsManager(this.UserCenterDbHelper, this.UserInfo, \"" + field + "\");"+ System.Environment.NewLine
                                            + "        DataTable dataTable" + field + " = manager" + field + ".GetDataTable(new KeyValuePair<string, object>(BaseItemDetailsEntity.FieldDeletionStateCode, 0), new KeyValuePair<string, object>(BaseItemDetailsEntity.FieldEnabled, 1), BaseItemDetailsEntity.FieldSortCode);" + System.Environment.NewLine
                                            + "        this.cmb" + field + ".DataSource = dataTable" + field + "; " + System.Environment.NewLine
                                            + "        this.cmb" + field + ".DataTextField = BaseItemDetailsEntity.FieldItemName; " + System.Environment.NewLine
                                            + "        this.cmb" + field + ".DataValueField = BaseItemDetailsEntity.FieldItemValue; " + System.Environment.NewLine
                                            + "        this.cmb" + field + ".DataBind(); " + System.Environment.NewLine
                                            + "        this.cmb" + field + ".Items.Insert(0, new ListItem());" + System.Environment.NewLine;
                            }
