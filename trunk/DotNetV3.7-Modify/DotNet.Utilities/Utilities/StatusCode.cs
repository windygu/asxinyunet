...
namespace DotNet.Utilities
{
    ...
    public enum StatusCode
    {
        ...
        SetPasswordOK = 34,         // 34 设置密码成功。
        OldPasswordError = 35,      // 35 原密码错误。
        ChangePasswordOK = 36,      // 36 修改密码成功。
        UserNotEmail = 37,          // 37 用户没有电子邮件地址。
        UserLocked = 38,            // 38 用户被锁定。
        UserNotActive = 39,         // 39 用户还未被激活。
        UserIsActivate = 40,        // 40 用户已被激活，不用重复激活。
        ErrorLogOn = 41,            // 41 用户名或密码错误。
        WaitForAudit = 42,          // 42 用户还在待审核状态。
        UserDuplicate = 43          // 43 用户还在待审核状态。
    }
    #endregion
}