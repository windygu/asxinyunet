...
namespace DotNet.Tools.ImageHelp
{
	public  static class ImageHelper
	{
		...
		public static bool ConvertImage(string OriFilename, string DesiredFilename)
		{
			...
			try
			{
				System.Drawing.Image imgFile = System.Drawing.Image.FromFile(Convertor.WebPathTran(OriFilename));
				imgFile.Save(Convertor.WebPathTran(DesiredFilename), DesiredFormat);
				return true;
			}
			catch
			{
		...
		{
			string SourceFileNameStr =Convertor.WebPathTran(OriFileName); //来源图片名称路径
			string TransferFileNameStr = Convertor.WebPathTran(DesiredFilename); //目的图片名称路径
