	...
	namespace YoungRunControl.Controls
	{
			...
			public class AddRawIntoFactory: UserControl
			{
				#region 自动生成代码
				...
				private void InitializeComponent()
				{
					...
					System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(AddRawIntoFactory));
					...
					#endregion
					#region 窗体
					this.Name = "AddRawIntoFactory";
					this.Size = new System.Drawing.Size(350, 500);
					this.Load += new System.EventHandler(this.AddAddRawIntoFactoryLoad);
					this.SuspendLayout();
			...
			#region 构造函数 及初始化
			public AddRawIntoFactory()
			{
				InitializeComponent();
			}
			//控件加载事件,完成数据绑定和相关基本设置
			void AddAddRawIntoFactoryLoad(object sender, EventArgs e)
			{				
			}
			/// <summary>
			/// 初始化设置
			...
			public string CutSearchCondition
			{
				...
				set
				{
					...
					FormPager.RecordCount = tb_RawIntoFactory.FindCount(CutSearchCondition,"","",0,0) ;
				}
			}
			/// <summary>
			/// 获取或者设置当前实体
			/// </summary>
			public tb_RawIntoFactory CutModel
			{
				get ;set ;
			}
			/// <summary>
			...
			//TODO:有问题，需要根据当前的状态来更新和保存数据
				...
				{
				tb_RawIntoFactory model = new tb_RawIntoFactory();//定义当前实体
				
					if(txtID.Text.Trim()!=""){
						model.ID = txtID.Text.Trim() ;}
				if(combOilNameTP.Text.Trim()!=""){
					model.OilNameTP = combOilNameTP.Text.Trim() ;}
				if(combProviderTP.Text.Trim()!=""){
					model.ProviderTP = combProviderTP.Text.Trim() ;}
					if(txtDataID.Text.Trim()!=""){
			...
			List <tb_RawIntoFactory> modelList ;
			void GetData()
			{				
				//判断不为空，才能绑定				
				modelList = tb_RawIntoFactory.FindAll(CutSearchCondition, "", "", (FormPager.PageIndex-1)*FormPager.PageSize ,FormPager.PageSize);
				if (modelList !=null ) {
			...
			private void SetControlReSet()
			{
				...
				if (CutShowMode == FormShowMode.AddOne || CutShowMode== FormShowMode.ContinueAdd )
				{
					...
					txtID.ReadOnly = false ;
							...
							combOilNameTP.Enabled = true ;
							combProviderTP.Enabled = true ;txtDataID.ReadOnly = false ;
			...
			private void SetControlEdit()
			{
				if (CutShowMode== FormShowMode.ContinueDisplay || CutShowMode== FormShowMode.DisplayCurrent ) {
					...
					btnOK.Text =" 保存";
					//控件除主键外都可读
				txtID.ReadOnly = true;combOilNameTP.Enabled = true ;combProviderTP.Enabled = true ;
			...
			private void SetControlReadOnly()
			{
				...
				txtID.ReadOnly = true ;
							...
							combProviderTP.Enabled = false ;
							txtDataID.ReadOnly = true ;
			...
			private void BandingData()
			{
				  ...
				  combProviderTP.DataBindings.Clear () ;
