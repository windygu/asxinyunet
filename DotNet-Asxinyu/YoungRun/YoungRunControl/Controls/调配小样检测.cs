	...
	namespace YoungRunControl.Controls
	{
			...
			/// 获取或者设置当前实体
			/// </summary>
			public tb_ReseachSample CutModel
			{
				get ;set ;
			}
			/// <summary>
			...
			List <tb_ReseachSample> modelList ;
			void GetData()
			{				
				//判断不为空，才能绑定				
			...
			private void SetControlEdit()
			{
				if (CutShowMode== FormShowMode.ContinueDisplay || CutShowMode== FormShowMode.DisplayCurrent ) {
					...
					//控件除主键外都可读
				txtID.ReadOnly = true;
					txtOilName.ReadOnly = false ;combProviderTP.Enabled = true ;
			...
			private void SetControlReadOnly()
			{
				...
				txtID.ReadOnly = true ;txtOilName.ReadOnly = true ;
