...
{
	public partial class KqTestDataManage: UserControl
	{
		#region 初始化		
		public KqTestDataManage()
		{
			InitializeComponent();
			string maininfo = "糠醛车间检测数据" ;
		...
		List < tb_kqtestdata > btList ;
		#endregion
		
		#region 菜单事件
		void GetData()
		{
			winPage.RecordCount = tb_kqtestdata.FindCount(cutSql ,"","",0,0) ;
			btList = tb_kqtestdata.FindAll (cutSql , "", "", (winPage.PageIndex -1)*winPage.PageSize ,
			                               winPage.PageSize );
		...
		{
            WinFormHelper.DeleteDgvItems<tb_kqtestdata >(sender, e,dgv,btList );
		...
		{
			AddKqTestDataForm kqform = new AddKqTestDataForm () ;
		...
		void ToolExportToExcelClick(object sender, EventArgs e)
		{
            ...
            sf.EntityName = typeof(tb_kqtestdata );
