...
namespace SVM
{
	...
	public class Problem
	{
		...
		/// <param name="stream">Stream to read from</param>
		/// <returns>The problem</returns>
		public static Problem ReadForSpecial(Stream stream)
		{
			TemporaryCulture.Start();
			StreamReader input = new StreamReader(stream);
			List<double> vy = new List<double>();
			...
			while (input.Peek() > -1)
			{
				...
				List<Node> cutX = new List<Node> () ;
				for (int j = 0; j < parts.Length -1; j++)
				{
					string[] nodeParts = parts[j + 1].Split(':');
					double temp = double.Parse(nodeParts[1]);
					if (temp !=0) //默认去掉0
					{
						Node k = new Node();
						k.Index = int.Parse(nodeParts[0]);
						k.Value = double.Parse(nodeParts[1]);
						cutX.Add (k ) ;
					}
				}
				m = cutX.Count ;
		...
		/// 根据数组读取问题
		/// </summary>
		/// <param name="CharacterResult">特征值-交错数组</param>
		/// <returns>问题模型</returns>
		public static Problem ReadForTestData(double[][] CharacterResult)
		{
			TemporaryCulture.Start();
			List<double> vy = new List<double>();
			List<Node[]> vx = new List<Node[]>();
			int max_index = 0;
			for (int i = 0; i < CharacterResult.GetLength (0); i++)
			{
				//string[] parts = input.ReadLine().Trim().Split();
				vy.Add(1);//测试数据默认添加1
				int m = CharacterResult[i].Length ;
				Node[] x = new Node[m];
				for (int j = 0; j < m; j++)
				{
					x[j] = new Node();
					if (CharacterResult[i][j]!=0) {
						x[j].Index = j +1 ;
						x[j].Value = CharacterResult[i ][j] ;
					}
				}
				if (m > 0)
					max_index = Math.Max(max_index, x[m - 1].Index) ;
		...
		public static Problem ReadForSpecial(string filename)
		{
			FileStream input = File.OpenRead(filename);
			try
			{
				return ReadForSpecial(input);
			}
			finally
			{
				input.Close();
