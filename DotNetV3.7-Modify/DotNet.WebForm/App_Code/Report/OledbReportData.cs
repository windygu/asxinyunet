...
public class OledbReportData
{
    ...
    //根据查询SQL,产生提供给报表生成需要的XML数据，采用 OleDb 数据引擎
    ...
    //根据查询SQL,产生提供给报表生成需要的XML数据，采用 OleDb 数据引擎, 这里只产生报表参数数据
    ...
    //根据查询SQL,产生提供给报表生成需要的XML数据，采用 OleDb 数据引擎, 根据RecordsetQuerySQL获取报表明细数据，根据ParameterQuerySQL获取报表参数数据
    ...
    //根据查询SQL,产生提供给报表生成需要的XML数据，采用 OleDb 数据引擎，字段值为空也产生数据
    ...
    //分批取数时，用 OleDbDataReader 产生报表XML数据
    //参数 SessionItemName 指定在 Session 中记录 OleDbDataReader 对象的名称，应该保证每个报表用不同的名称
    ...
    public static void BatchGenXmlDataByDataReader(System.Web.UI.Page DataPage, string SessionItemName, string QuerySQL, int StartNo, int WantRows, bool ToCompress)
    {
        ...
        OleDbDataReader myReader = (OleDbDataReader)DataPage.Session[SessionItemName];
