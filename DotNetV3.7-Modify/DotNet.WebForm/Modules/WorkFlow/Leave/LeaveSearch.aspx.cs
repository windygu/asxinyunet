...
/// </author>
/// </remarks>
public partial class LeaveSearch : BasePage
{
	/// <summary>
    /// 是否按工作流方式运行
    /// </summary>
    ...
    private void GetPermission()
    {
        ...
        /*
        this.permissionDelete = this.IsAuthorized("Leave.Delete");
        */
    }
    #endregion
    ...
    private void DoSearch()
    {
        ...
        // 总记录数量
        int recordCount = 0;
        LeaveManager manager = new LeaveManager(this.UserInfo);
        DataTable dataTable = manager.GetDataTableByPage(string.Empty, this.txtSearch.Text, out recordCount, this.myNavigator.PageIndex, this.PageSize, this.SortExpression, this.SortDire);
    ...
    private void GetDataBaseParamter()
    {
        ...
        string pageSize = manager.GetParameter("User", this.UserInfo.Id, "Leave.PageSize");
    ...
    {
        BaseParameterManager manager = new BaseParameterManager(this.DbHelper, this.UserInfo);
        manager.SetParameter("User", this.UserInfo.Id, "Leave.PageSize", this.myNavigator.PageSize.ToString());
    }
    #endregion
        ...
        if (e.Row.RowType == DataControlRowType.DataRow || e.Row.RowState == DataControlRowState.Normal && e.Row.RowState == DataControlRowState.Alternate)
        {
			...
			if (!this.permissionDelete)
			{
					...
					|| e.Row.RowState == (DataControlRowState.Alternate))
				{
					...
					string enabled = dataRowView[LeaveEntity.FieldEnabled].ToString();
					bool allowEdit = true;
    ...
    private void ShowEdit(string id)
    {
        ...
        {
			// string url = "LeaveEdit.aspx?Id=" + id + "&ReturnURL=LeaveSearch.aspx&ticks=" + ticks;
            string url = "LeaveEdit.aspx?Id=" + id + "&ticks=" + ticks;
    ...
    private void ShowDetail(string id)
    {
        ...
        if (!string.IsNullOrEmpty(id))
        {
            // string url = "LeaveShow.aspx?Id=" + id + "&ReturnURL=LeaveSearch.aspx&ticks=" + ticks;
			string url = "LeaveShow.aspx?Id=" + id + "&ticks=" + ticks;
			this.ClientScript.RegisterStartupScript(this.GetType(), "OPEN_WINDOW", "<script>window.open('" + url + "');</script>");
			// Page.Response.Redirect(url);
        }
    }
    ...
    {
	    // string url = "LeaveEdit.aspx?ReturnURL=LeaveSearch.aspx";
		string url = "LeaveEdit.aspx";
		this.ClientScript.RegisterStartupScript(this.GetType(), "OPEN_WINDOW", "<script>window.open('" + url + "');</script>");
        // Page.Response.Redirect(url);
    }
