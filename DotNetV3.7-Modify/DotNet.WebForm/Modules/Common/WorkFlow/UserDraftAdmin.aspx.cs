...
/// </author>
/// </remarks>
public partial class UserDraftAdmin : BasePage
{
    ...
    private void SetControlState()
    {
        ...
        {
            this.btnBatchAutoStatr.Enabled = false;
        ...
        else
        {
            ...
            this.btnBatchAutoStatr.Enabled = this.permissionEdit;
    ...
    private void GetItemDetails()
    {
        ...
        parameters.Add(new KeyValuePair<string, object>(BaseNewsEntity.FieldEnabled, 1));
        parameters.Add(new KeyValuePair<string, object>(BaseNewsEntity.FieldDeletionStateCode, 0));
        DataTable dataTable = templateManager.GetDataTable(parameters, BaseNewsEntity.FieldSortCode + " DESC");
        this.cmbCategory.DataSource = dataTable;
        this.cmbCategory.DataTextField = BaseNewsEntity.FieldTitle;
        this.cmbCategory.DataValueField = BaseNewsEntity.FieldCode;
    ...
    private void DoPageLoad()
    {
        try
        {
            ...
            // 这里不需要判断权限
            // this.IsModuleAuthorized("UserBillAdmin.aspx");
            this.GetItemDetails();
            this.GetParamters();
    ...
    private void DoSearch()
    {
        ...
        DataTable dataTable = newsManager.SearchBill(this.UserInfo.Id, string.Empty, this.cmbCategory.SelectedValue, search, false, false);
    ...
    protected void gridView_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow || e.Row.RowState == DataControlRowState.Normal && e.Row.RowState == DataControlRowState.Alternate)
        {
            ...
            if (e.Row.Cells[6].Text != "&nbsp;" && !e.Row.Cells[6].Text.Equals("草稿"))
            {
                ...
                e.Row.Cells[3].Text = ((DataRowView)e.Row.DataItem)[BaseNewsEntity.FieldTitle].ToString();
            }
            else
            {
    ...
    protected void gridView_RowEditing(object sender, GridViewEditEventArgs e)
    {
        ...
        BaseNewsEntity entity = billManager.GetEntity(id);
        // 导向编辑页面
        WorkFlowBillTemplateManager templateManager = new WorkFlowBillTemplateManager(this.WorkFlowDbHelper, this.UserInfo);
        string editPage = templateManager.GetEditPage(entity.FolderId);
        editPage = editPage.Replace("{Id}", id);
        this.ClientScript.RegisterStartupScript(this.GetType(), "OPEN_WINDOW", "<script>window.open('" + editPage + "');</script>");
        // this.DoSearch();
    }
    ...
    {
        Page.Response.Redirect("SelectTemplate.aspx");
        // string url = "UserHtmlBillEdit.aspx";
        // this.ClientScript.RegisterStartupScript(this.GetType(), "OPEN_WINDOW", "<script>window.open('" + url + "');</script>");
    }

    protected int BatchAutoStatr()
    {
        int returnValue = 0;
        ...
        try
        {
            ...
            // returnValue = workFlowCurrentManager.AutoStatr(IWorkFlowManager workFlowManager, string objectId, string objectFullName, string categoryCode, string categoryFullName = null);
        ...
        if (Utilities.ShowInformation)
        {
            if (returnValue > 0)
            {
                this.ClientScript.RegisterStartupScript(this.GetType(), "bathAllowSucceed", "<script>alert('提示信息：单据提交成功。');</script>");
            }
            else
            {
                this.ClientScript.RegisterStartupScript(this.GetType(), "bathAllowFailure", "<script>alert('提示信息：单据提交失败。');</script>");
            }
        }
        return returnValue;
    }

    protected void btnBatchAutoStatr_Click(object sender, EventArgs e)
    {
        this.BatchAutoStatr();
    }
        ...
        try
        {
            ...
            for (int i = 0; i < ids.Length; i++)
            {
                ...
                if (string.IsNullOrEmpty(auditStatus) 
                    ...
                    || auditStatus.Equals(AuditStatus.AuditQuash.ToString()))
                {
