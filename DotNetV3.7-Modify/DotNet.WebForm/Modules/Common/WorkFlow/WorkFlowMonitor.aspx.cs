...
/// </author>
/// </remarks>
public partial class WorkFlowMonitor : BasePage
{
    ...
    private void DoPageLoad()
    {
        try
        {
            ...
            // 这里需要判断权限
            this.IsModuleAuthorized("WorkFlowMonitor");
            this.GetItemDetails();
            this.GetParamters();
    ...
    private void DoSearch()
    {
        ...
        DataTable dataTable = workFlowCurrentManager.GetMonitorDT(this.cmbCategory.SelectedValue, search);
    ...
    protected int BatchReset()
    {
        ...
        try
        {
            ...
            {
                // returnValue += newsManager.Reset(ids[i], string.Empty);
            }
            this.DoSearch();
        }
        catch (Exception ex)
        ...
        if (Utilities.ShowInformation)
        {
            if (returnValue > 0)
            {
                this.ClientScript.RegisterStartupScript(this.GetType(), "batchResetSucceed", "<script>alert('提示信息：重置单据成功。');</script>");
            }
            else
            {
                this.ClientScript.RegisterStartupScript(this.GetType(), "batchResetFailure", "<script>alert('提示信息：重置单据失败。');</script>");
            }
        }
        return returnValue;
    }
