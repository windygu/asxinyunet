...
/// </author>
/// </remarks>
public partial class ProcessingMaintain : BasePage
{
    ...
    private void SetControlState()
    {
        ...
        this.btnSetDraft.Enabled = this.gridView.Rows.Count > 0;
    ...
    private void DoSearch()
    {
        ...
        // DataTable dataTable = maintainManager.GetDataTableByPage(this.UserInfo.Id, "维修中", searchValue, out recordCount, this.myNavigator.PageIndex + 1, this.myNavigator.PageSize, this.SortExpression, this.SortDire);
        DataTable dataTable = maintainManager.GetDataTableByPage(this.UserInfo.Id, string.Empty, MaintainStatus.MaintainProcessing.ToDescription(), searchValue, out recordCount, this.myNavigator.PageIndex + 1, this.myNavigator.PageSize, this.SortExpression, this.SortDire);
    ...
    #region private void GetList(PageChangeEventArgs e) 绑定信息
    /// <summary>
    /// 绑定信息
    /// </summary>
    ...
    #region private int SetDraft() 批量设置为草稿
    /// <summary>
    /// 批量设置为草稿
    /// </summary>
    /// <returns>影响的行数</returns>
    private int SetDraft()
    {
        int returnValue = 0;
        string[] ids = Utilities.GetSelecteIds(this.gridView);
        MaintainManager manager = new MaintainManager(this.UserInfo);
        returnValue = manager.SetDraft(ids);
        this.Search();
        return returnValue;
    }
    #endregion

    protected void btnSetDraft_Click(object sender, EventArgs e)
    {
        // 批量撤销
        this.SetDraft();
    }

    #region private int SetProcessed() 批量已处理
    /// <summary>
    /// 批量已处理
    /// </summary>
    /// <returns>影响的行数</returns>
