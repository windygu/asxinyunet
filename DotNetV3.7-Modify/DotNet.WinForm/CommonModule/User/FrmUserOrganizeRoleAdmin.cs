...
{    
    ...
    public partial class FrmUserOrganizeRoleAdmin : BaseForm
    {
        ...
        private void LoadTreeOrganize(TreeNode treeNode)
        {
            ...
            {
                dataRows = this.DTOrganize.Select(BaseOrganizeEntity.FieldParentId + "='" + treeNode.Tag.ToString() + "'");
            }
            foreach (DataRow dataRow in dataRows)
            {
                // 判断不为空的当前节点的子节点
                if ((treeNode.Tag != null) && (treeNode.Tag.ToString().Length > 0) && (!treeNode.Tag.Equals(dataRow[BaseOrganizeEntity.FieldParentId].ToString())))
                {
                    continue;
                }
                // 当前节点的子节点, 加载根节点
                if ((dataRow.IsNull(BaseOrganizeEntity.FieldParentId) || (dataRow[BaseOrganizeEntity.FieldParentId].ToString().Length == 0) || (dataRow[BaseOrganizeEntity.FieldParentId].ToString().Equals(BaseSystemInfo.RootMenuCode)) || ((treeNode.Tag != null) && treeNode.Tag.Equals(dataRow[BaseModuleEntity.FieldParentId].ToString()))))
                {
                    ...
                    DataRow[] childRows = this.DTRole.Select(BaseRoleEntity.FieldSystemId + "='" + newTreeNode.Tag.ToString() + "'");
        ...
        private void EntityToDataTable(TreeNode treeNode)
        {
            ...
            {
                // 提高运行速度
                DataRow[] dataRows = this.DTRole.Select(BaseRoleEntity.FieldId + "='" + treeNode.Tag.ToString() + "'");
                foreach (DataRow dataRow in dataRows)
                {
                ...
                // BUBaseBusinessLogic.SetProperty(this.DSOrganize.Tables[BUBaseUserRole.TableName], "RoleId", TreeNode.Tag.ToString(), "colSelected", TreeNode.Checked ? 1 : 0);
            }
            for (int i = 0; i < treeNode.Nodes.Count; i++)
            {
                // 这里进行递规操作
