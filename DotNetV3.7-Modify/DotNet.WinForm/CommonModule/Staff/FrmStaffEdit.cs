...
{  
    ...
    public partial class FrmStaffEdit : BaseForm
    {
        ...
        private void btnSetStaffUser_Click(object sender, EventArgs e)
        {
            ...
            if (this.staffEntity.UserId > 0)
            {
                ((IBaseSelectUserForm)frmUserSelect).OpenId = this.staffEntity.UserId.ToString();
            }
            ((IBaseSelectUserForm)frmUserSelect).AllowNull = true;
            ((IBaseSelectUserForm)frmUserSelect).MultiSelect = false;
            if (frmUserSelect.ShowDialog() == DialogResult.OK)
            {
                string userId = ((IBaseSelectUserForm)frmUserSelect).SelectedId;
