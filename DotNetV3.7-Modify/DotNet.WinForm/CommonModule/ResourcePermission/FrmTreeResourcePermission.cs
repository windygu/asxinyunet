...
namespace DotNet.WinForm
{
    ...
    public partial class FrmTreeResourcePermission : BaseForm
    {
        ...
        private void LoadTreeModule(TreeNode treeNode)
        {
            ...
            {
                // 判断不为空的当前节点的子节点
                if ((treeNode.Tag != null) && (treeNode.Tag.ToString().Length > 0) && (!treeNode.Tag.Equals(dataRow[this.FieldParentId].ToString())))
                {
                    continue;
                }
                // 当前节点的子节点, 加载根节点
                if ((dataRow.IsNull(this.FieldParentId) || (dataRow[this.FieldParentId].ToString().Length == 0) || (dataRow[this.FieldParentId].ToString().Equals(BaseSystemInfo.RootMenuCode)) || ((treeNode.Tag != null) && treeNode.Tag.Equals(dataRow[this.FieldParentId].ToString()))))
                {
                    TreeNode newTreeNode = new TreeNode();
                    newTreeNode.Text = dataRow[this.FieldRealName].ToString();
                    newTreeNode.Tag = dataRow[this.FieldId].ToString();
                    // 是否已经有这个权限
                    newTreeNode.Checked = Array.IndexOf(this.SetSelectIds, newTreeNode.Tag.ToString()) >= 0;
        ...
        private void EntityToArray(TreeNode treeNode)
        {
            ...
            {
                // 提高运行速度
                string resourceId = treeNode.Tag.ToString();
                if (treeNode.Checked)
                {
                    // 这里是授权的权限
