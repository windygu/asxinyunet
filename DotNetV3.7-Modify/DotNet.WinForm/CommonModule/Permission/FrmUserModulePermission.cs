...
namespace DotNet.WinForm
{
    ...
    /// <summary>
    /// FrmUserModulePermission.cs
    /// 模块管理-模块配置
    ///		
    /// 修改记录
    ///     2011.10.31 版本：2.2 张广梁    优化了节点选择
    ...
    public partial class FrmUserModulePermission : BaseForm
    {
        ...
        public FrmUserModulePermission(string userId, string userRealame) : this()
        {
        ...
        private void LoadTreeModule(TreeNode treeNode)
        {
            foreach (DataRow dataRow in this.DTModule.Rows)
            {
                ...
                if ((dataRow.IsNull(BaseModuleEntity.FieldParentId) || (dataRow[BaseModuleEntity.FieldParentId].ToString().Length == 0) || (dataRow[BaseModuleEntity.FieldParentId].ToString().Equals("0")) || (dataRow[BaseModuleEntity.FieldParentId].ToString().Equals(BaseSystemInfo.RootMenuCode)) || ((treeNode.Tag != null) && treeNode.Tag.Equals(dataRow[BaseModuleEntity.FieldParentId].ToString()))))
                {
                    ...
                    // 是否已经有这个权限
                    newTreeNode.Checked = Array.IndexOf(this.moduleIds, newTreeNode.Tag.ToString()) >= 0;
        ...
        //{
            // 是用户点了复选框。改用在tvModule_NodeMouseClick函数中处理了，edit by zgl on 2011-10-31
            //if (this.isUserClick)
            //{
