...
namespace DotNet.WinForm
{
    ...
    /// <summary>
    /// FrmUserPermissionItemScope.cs
    /// 用户－可分配授权的权限设定
    ///		
    /// 修改记录
    ///
    ///     2009.09.28 版本：1.0 JiRiGaLa 权限的显示顺序进行优化。
    ///     2008.05.25 版本：1.0 JiRiGaLa 用户－可分配授权的权限设定。
    ///		
    /// 版本：1.0
    ///
    /// <author>
    ...
    /// </author> 
    /// </summary>  
    public partial class FrmUserPermissionItemScope : BaseForm
    {
        ...
        public string CurrentEntityId
        {
            get
            {
                ...
                {
                    this.currentEntityId = this.tvPermission.SelectedNode.Tag.ToString();
                }
        ...
        public FrmUserPermissionItemScope()
        {
            InitializeComponent();
        }

        public FrmUserPermissionItemScope(string currentId) : this()
        {
            this.CurrentEntityId    = currentId;
            this.OldEntityId        = currentId;
        }

        public FrmUserPermissionItemScope(string userId, string fullName) : this()
        {
        ...
        public override void SetControlState()
        {
            ...
            if (String.IsNullOrEmpty(this.TargetUserId) || this.DTPermissionItem.Rows.Count == 0)
            {
        ...
        {
            this.ucUser.SelectedId = this.TargetUserId;
            this.ucUser.SelectedFullName = this.TargetName;
            // 加载树
            if (reloadTree)
            {
                this.LoadTree();
        ...
        public override void FormOnLoad()
        {
            ...
            this.PermissionIds = DotNetService.Instance.PermissionService.GetUserScopePermissionItemIds(UserInfo, this.TargetUserId, this.PermissionItemScopeCode);
            // 设置鼠标默认状态，原来的光标状态
            this.BindData(true);
        }
        #endregion
        ...
        private void LoadTree(TreeNode treeNode)
        {
            ...
            {
                dataRows = this.DTPermissionItem.Select(BasePermissionItemEntity.FieldParentId + " IS NULL OR " + BasePermissionItemEntity.FieldParentId + " = 0");
            }
            else
            {
                dataRows = this.DTPermissionItem.Select(BasePermissionItemEntity.FieldParentId + "='" + treeNode.Tag.ToString() + "'", BasePermissionItemEntity.FieldSortCode);
            }
            foreach (DataRow dataRow in dataRows)
            {
                // 判断不为空的当前节点的子节点
                if ((treeNode.Tag != null) && (treeNode.Tag.ToString().Length > 0) && (!treeNode.Tag.Equals(dataRow[BasePermissionItemEntity.FieldParentId].ToString())))
                {
                    continue;
                }
                // 当前节点的子节点, 加载根节点
                if ((dataRow.IsNull(BasePermissionItemEntity.FieldParentId) || (dataRow[BasePermissionItemEntity.FieldParentId].ToString().Length == 0) || ((treeNode.Tag != null) && treeNode.Tag.Equals(dataRow[BasePermissionItemEntity.FieldParentId].ToString()))))
                {
        ...
        private bool BatchSave()
        {
            ...
            DotNetService.Instance.PermissionService.GrantUserPermissionItemScopes(UserInfo, this.TargetUserId, this.PermissionItemScopeCode, grantPermissionIds);
            DotNetService.Instance.PermissionService.RevokeUserPermissionItemScopes(UserInfo, this.TargetUserId, this.PermissionItemScopeCode, revokePermissionIds);
