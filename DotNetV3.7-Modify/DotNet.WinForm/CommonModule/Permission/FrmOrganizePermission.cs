...
namespace DotNet.WinForm
{
    ...
    /// <summary>
    /// FrmOrganizePermission.cs
    /// 权限设置-组织机构设置
    ///		
    /// 修改记录
    /// 
    ///     2012.03.22 版本：1.0 JiRiGaLa  组织机构权限设置。
    ///		
    /// 版本：1.0
    ///
    /// <author>
    ...
    public partial class FrmOrganizePermission : BaseForm
    {
        ...
        {
            // 获取模块访问权限
            this.ModuleIds = DotNetService.Instance.PermissionService.GetOrganizeScopeModuleIds(UserInfo, this.TargetOrganizeId, "Resource.AccessPermission");
            ...
            // 获得角色的权限主键数组
            this.PermissionItemIds = DotNetService.Instance.PermissionService.GetOrganizePermissionItemIds(UserInfo, this.TargetOrganizeId);
        ...
        private void tvModule_AfterCheck(object sender, TreeViewEventArgs e)
        {
            if (this.isUserClick)
            {
                ...
                {
                    // 授予操作权限
                    DotNetService.Instance.PermissionService.GrantOrganizeModuleScope(this.UserInfo, this.TargetOrganizeId, "Resource.AccessPermission", e.Node.Tag.ToString());
                }
                else
                {
                    // 撤销操作权限
                    DotNetService.Instance.PermissionService.RevokeOrganizeModuleScope(this.UserInfo, this.TargetOrganizeId, "Resource.AccessPermission", e.Node.Tag.ToString());
                }
        ...
        private void tvPermissionItem_AfterCheck(object sender, TreeViewEventArgs e)
        {
            if (this.isUserClick)
            {
                ...
                {
                    // 授予操作权限
                    DotNetService.Instance.PermissionService.GrantOrganizePermissionById(this.UserInfo, this.TargetOrganizeId, e.Node.Tag.ToString());
                }
                else
                {
                    // 撤销操作权限
                    DotNetService.Instance.PermissionService.RevokeOrganizePermissionById(this.UserInfo, this.TargetOrganizeId, e.Node.Tag.ToString());
                }
        ...
        private void btnClearPermission_Click(object sender, EventArgs e)
        {
            ...
            {
                // 清除角色权限
                DotNetService.Instance.PermissionService.ClearOrganizePermission(this.UserInfo, this.TargetOrganizeId);
                this.GetCurrentPermission();
            }
        }
