...
namespace DotNet.Business
{
    ...
    public partial class PermissionService : System.MarshalByRefObject, IPermissionService
    {
        ...
        #region public bool IsInRole(BaseUserInfo userInfo, string userId, string roleName) 用户是否在指定的角色里
        /// <summary>
        /// 用户是否在指定的角色里
        /// </summary>
            ...
            using (IDbHelper dbHelper = DbHelperFactory.GetHelper(BaseSystemInfo.UserCenterDbType))
            {
                try
                {
                    ...
                    if (userManager.IsAdministrator(userId))
                    {
                        ...
                        dataTable = permissionAdminManager.GetDataTable();
                    }
                    else
                    {
                        BasePermissionManager permissionManager = new BasePermissionManager(dbHelper);
