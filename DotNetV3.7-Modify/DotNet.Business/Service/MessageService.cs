...
namespace DotNet.Business
{
    ...
    public class MessageService : System.MarshalByRefObject, IMessageService
    {
        ...
        public DataTable GetUserDTByOrganize(BaseUserInfo userInfo, string organizeId)
        {
            ...
            using (IDbHelper dbHelper = DbHelperFactory.GetHelper(BaseSystemInfo.UserCenterDbType))
            {
                try
                {
                    ...
                    {
                        // 从用户表
                        sqlQuery += " AND ((" + BaseUserEntity.TableName + "." + BaseUserEntity.FieldDepartmentId + " = '" + organizeId + "' AND " + BaseUserEntity.TableName + "." + BaseUserEntity.FieldWorkgroupId + " IS NULL ";
                        sqlQuery += "       OR (" + BaseUserEntity.TableName + "." + BaseUserEntity.FieldWorkgroupId + " = '" + organizeId + "')) ";
                        // 从兼职表读取用户
        ...
        #region public string Send(BaseUserInfo userInfo, string receiverId, string content) 发送消息
        /// <summary>
        /// 发送消息
        /// </summary>
        /// <param name="userInfo">用户</param>
        ...
        public string Send(BaseUserInfo userInfo, string receiverId, string content)
        {
            ...
            using (IDbHelper dbHelper = DbHelperFactory.GetHelper(BaseSystemInfo.UserCenterDbType))
            {
                try
                {
                    ...
                    messageEntity.Contents = content;
