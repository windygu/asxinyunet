...
namespace DotNet.Business
{
    ...
    public class FileService : System.MarshalByRefObject, IFileService
    {
        ...
        public string Send(BaseUserInfo userInfo, string fileName, byte[] file, string toUserId)
        {
            ...
            using (IDbHelper dbHelper = DbHelperFactory.GetHelper(BaseSystemInfo.UserCenterDbType))
            {
                try
                {
                    ...
                    // 02:判断用户的空间是否存在？
                    if (!folderManager.Exists(userInfo.Id))
                    {
                    ...
                    // 03:判断已经已发文件是否存在？
                    if (!folderManager.Exists(userInfo.Id + "_Send"))
                    {
                    ...
                    //if (!folderManager.Exists(userInfo.Id + "_Receive"))
                    //{
                    //    folderEntity.FolderName = "收到檔案夾";
                    //    folderEntity.ParentId = userInfo.Id;
                    ...
                    if (!string.IsNullOrEmpty(userEntity.Id))
                    {
                        ...
                        //if (!folderManager.Exists(toUserId + "_Send"))
                        //{
                        //    folderEntity.FolderName = "發送資料夾";
                        //    folderEntity.ParentId = toUserId;
                        //    folderEntity.Id = toUserId + "_Send";
                        //    folderManager.AddEntity(folderEntity);
                        //}
        ...
        #region public string Add(BaseUserInfo userInfo, string folderId, string fileName, byte[] file, string description, string category, bool enabled, out string statusCode, out string statusMessage)
        /// <summary>
        /// 添加
        /// </summary>
        /// <param name="userInfo">用户</param>
        ...
        public string Add(BaseUserInfo userInfo, string folderId, string fileName, byte[] file, string description, string category, bool enabled, out string statusCode, out string statusMessage)
        {
            ...
            using (IDbHelper dbHelper = DbHelperFactory.GetHelper(BaseSystemInfo.UserCenterDbType))
            {
                try
                {
                    dbHelper.Open(UserCenterDbConnection);
                    BaseFileManager fileManager = new BaseFileManager(dbHelper, userInfo);
                    returnValue = fileManager.Add(folderId, fileName, file, description, category, enabled, out statusCode);
