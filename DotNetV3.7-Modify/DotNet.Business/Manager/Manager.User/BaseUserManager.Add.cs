...
namespace DotNet.Business
{
    ...
    public partial class BaseUserManager : BaseManager
    {
        ...
        public void BeforeAdd(BaseUserEntity userEntity, out string statusCode)
        {
            ...
            if (this.Exists(new KeyValuePair<string, object>(BaseUserEntity.FieldUserName, userEntity.UserName), new KeyValuePair<string, object>(BaseUserEntity.FieldDeletionStateCode, "0"))
                || DbLogic.Exists(DbHelper, BaseStaffEntity.TableName, parameters))
            {
                // 用户名已重复
            ...
            {
                // 检查编号是否重复
                if (!string.IsNullOrEmpty(userEntity.Code) && this.Exists(new KeyValuePair<string, object>(BaseUserEntity.FieldCode, userEntity.Code), new KeyValuePair<string, object>(BaseUserEntity.FieldDeletionStateCode, "0")))
                {
                    // 编号已重复
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(userEntity.Code) && DbLogic.Exists(DbHelper, BaseStaffEntity.TableName, parameters))
                    {
                        // 编号已重复
                        statusCode = StatusCode.ErrorCodeExist.ToString();
                    }
